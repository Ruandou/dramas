...
- **R2 视觉资产模式**：当 **Stage 3 完成后（G3 通过，道具/角色/场景资产全部就绪）** 时，满分调整为 35 分（新增"视觉资产匹配度"维度），EP01 硬门槛调整为 ≥25/35，EP02+ 软门槛调整为 ≥21/35
...
2. **时长不达标即重写整集**：剧本总时长低于或高于 `制片规范.md` → `episode_profile` 定义的合规区间（示例 standard-72: <75s/EP01<90s 或 >120s；长集项目以实际 episode_profile 为准）时，**不得**通过延长/拆分/加镜头等方式在原稿上补足。唯一路径：废弃当前稿 → 重做时长预算 → 从大纲重新展开整集。"只差几秒微调一下"的念头必须打消，不分差距大小，差 1s 也重写。
