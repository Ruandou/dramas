...
| 反 AI 模式 | 全集中无超过 2 行同一开头词、无 3 行连续相同句式、无套话模板重复；无 Rule 36(h) 语义模板黑名单命中（结巴表怯弱/单字隐忍独白/倒计时深情句/排比宣言）；主角判词腔 cadence ≤3 次且金句 ≤4 句（Rule 47b/34，防"句句端着"生硬） | 请求 scene-writer 改写为更角色化的表达（超配额判词降格为功能性平话） |
