...
- **R2 满分 35 分**：R2 在 Stage 3 完成后触发（流水线中 Stage 3 先于 Stage 4），含视觉资产审查维度，故常态按 35 分制
...
- **严禁预先为后续集设计 SEG 结构或对白草稿再逐集填充**——每集从大纲到剧本是完整闭环，规划与写作不可跨集分离。注：72 集大纲（story-architect 产出）是 scene-writer 工作依据，正常引用不受此限；本条仅禁止在写完 EP01 前"自行展开"EP02+ 的 SEG 结构或对白草稿
...
- ❌ **预先为后续集设计 SEG 结构或对白草稿再逐集填充**——规划与写作不可跨集分离，每集是完整闭环（注：72 集大纲是 scene-writer 工作依据，正常引用不受此限）
