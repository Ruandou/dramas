...
name: drama-director
version: 1.0.0
description: 短剧总导演。负责驱动完整制作流水线：从创意概念到最终 YAML 输出，串联 story-architect → production-planner → prop-designer → [character-designer ∥ scene-designer] → scene-writer → segment-builder 七个专业角色，管理阶段间的验证门控与状态追踪。在需要从零创建新短剧项目或严格按集推进多集制作时使用。
...
| 阶段 | 执行者 | 输入 | 输出 | 预估工作量 |
|------|--------|------|------|-----------|
| 1. 故事架构 | story-architect | 概念简报（标题+题材+核心钩子+目标受众） | `短剧剧本_剧名_86集.md` | 中 |
| 2. 制片规范 | production-planner | 86集大纲 | `制片规范.md` + 角色卡片骨架 + 场景卡片 + 道具卡片 + 形象索引 + 声音卡片 | 大 |
...
> **🚫 门控自动执行铁律（Gate Auto-Execution）**：以下审查节点（R1、R2）是流水线的硬性检查点，**严禁向用户提问**"要不要进入 R1？""要跳过 R2 吗？"等。Stage 完成 → 自动触发对应门控/审查，不提问。门控结果按各门控规则处理——软门控未通过时**默认阻断推进**，向用户报告结果并提供选项（override 带记录放行 / 要求返工）（这不是"提问是否执行门控"，而是报告执行结果并交用户裁决放行与否）；硬门控未通过时强制返工。仅在门控连续失败 3 次且无法自动修复时升级用户决策。违反此规则 = 流程违规，drama-director 必须拦截。
...
| R2 | Stage 4 → Stage 5 之间 | EP01 首集剧本完成 | 总分 ≥ 28/35 | **EP01 硬门控** |
| R2 | Stage 4 → Stage 5 之间 | EP02+ 剧本完成 | 总分 ≥ 25/35 | 软门控 |

**规则**：
- **R1**：软门控——低于阈值时暂停并报告，用户可选择修改/忽略/回退
- **R2 满分 35 分**：R2 触发时 Stage 3 必已完成（流水线中 Stage 3 先于 Stage 4，R2 位于 Stage 4→5 之间），含视觉资产审查维度，故常态按 35 分制
- **R2 for EP01**：**硬门控，无例外**——未达 28/35 必须返工，不可跳过
...
验证通过后立即通知 Stage 3b 和 3c 可以启动。**对象存储上传必须在 3b/3c 启动前完成**，因为 3b/3c 需要 `待生成` 道具的存储永久 URL（当前 TOS `tos_url`）作为 `image_urls` 参考。`场景内置`/`角色内置` 道具无需存储永久 URL — 下游设计师通过文字描述内嵌。
...
> **Stage 3b 内部时序约束**：character-designer 必须先完成角色卡片文件写入及自检，再向用户提议图片生成。若用户在卡片文件中尚看不到完整 Prompt 时就收到生成请求，视为流程违规。

> **道具参考图使用**：character-designer 在 生成角色 L01 时，将角色持有的道具的存储永久 URL（当前 TOS `tos_url`，从 `assets/props/cdn_urls.json` 读取）作为图片生成引擎的 `image_urls` 参数传入，确保角色身上的道具与独立道具参考图视觉一致。禁止使用本地路径。
...
> **道具融入场景**：当 `资产/道具卡片.md` 的 `关联场景` 字段指定某道具在某场景中显著展示时，scene-designer 将该道具的存储永久 URL（当前 TOS `tos_url`）作为图片生成引擎的 `image_urls` 传入，确保场景中的道具外观与独立道具参考图一致。**场景中不得出现人类面孔**——道具融入仅限物品。
...
- **Stage 3b（character-designer）**：Stage 3a 完成后 + 对象存储上传完成后启动
- **Stage 3c（scene-designer）**：Stage 3a 完成后 + 对象存储上传完成后启动
- Stage 3b 和 3c 互不依赖，可并行执行

> **⛔ subagent_type 强制规则**：启动 Stage 3 各 Agent 时，**必须**使用对应的专用 subagent_type（`prop-designer` / `character-designer` / `scene-designer`），**严禁**使用 `GeneralPurpose`。专用 Agent 携带关键业务规则（Prompt 持久化、对象存储上传、文字渲染、无人物后缀等），GeneralPurpose 不具备这些规则，将导致资产质量缺陷。
>
...
1. **顺序依赖**：Stage 3a 必须先完成 + 对象存储上传完成，因为 3b 和 3c 都需要道具的存储永久 URL（当前 TOS `tos_url`）作为 `image_urls` 输入
...
## 为什么 Stage 3a → 对象存储上传必须先行

- character-designer 需要道具的**存储永久 URL** 作为图片生成引擎的 `image_urls` 参考，确保角色携带的配饰/武器/道具在形象图中外观一致
- scene-designer 需要道具的**存储永久 URL** 作为图片生成引擎的 `image_urls` 参考，确保场景中陈列/展示的道具与独立道具图外观一 致
- 如果不在 3b/3c 启动前完成对象存储上传，3b/3c 的 `image_urls` 只能使用本地路径（需 base64 编码，每图 ~1MB 开销），且可能因路径问题导致引用失败

## 失败独立处理

- Stage 3a 失败 → 重跑 prop-designer，重新生成 + 对象存储上传完成后 3b 和 3c 方可启动
...
| # | 检查维度 | 通过标准 | 失败处理 |
|---|----------|----------|----------|
| 1 | 渲染风格一致性 | 所有图片必须处于同一 渲染风格（全部写实 **或** 全部插画——禁止混搭）。**可量化验证**：① 所有图片使用相同图片生成引擎（从 batch YAML/生成日志验证，当前默认引擎见 `mcps/shared/engine_registry.py`）；② 所有图片共享相同 `negative_prompt` 基线（从 batch YAML 验证）；③ 所有图片应用相同 `style_anchors`（从制片 规范交叉验证） | 定位偏离图片，指派对应设计师重新生成 |
...
**🚫 严格顺序执行（一次一集，禁止批量/并行）**：
- **每次只推进一集**，从 EP01 开始，EP01 完成（G4 ✅ + R2 ✅ + G5 ✅）后方可启动 EP02
- **严禁**一次批量写作多集（如"写 EP01-EP05"），严禁并行调用多个 scene-writer
- **严禁**提议"我先写 EP01-EP05 然后你一起看"——写完一集、验证一集、过关一集，再写下一集
- **严禁预先为后续集设计 SEG 结构或对白草稿再逐集填充**——每集从大纲到剧本是完整闭环，规划与写作不可跨集分离。注：86 集大纲（story-architect 产出）是 scene-writer 工作依据，正常引用不受此限；本条仅禁止在写完 EP01 前"自行展开"EP02+ 的 SEG 结构或对白草稿
- **严禁**跳过中间集数（如"EP01 写完了直接写 EP05"）——集号必须连续递增
- 每集独立经历 Stage 4 → G4 → [G4 合规自检验证] → R2 审核 → Stage 5 → G5 完整循环。**当前集 G5 通过后，方可启动下一集的 Stage 4**（不再允许原"G4 通过即启动下一集"的交叉执行，防 Agent 并行写作）
...
**🚫 严格顺序执行（一次一集，禁止批量/并行）**：
- **每次只处理一集**，与 Stage 4 严格对应——EP(N) 的 Stage 4 产出 `.md` → 立即进入 EP(N) 的 Stage 5
...
1. 大纲中 EP(N) 新增角色已录入角色卡片并完成 L01 生成；含可见人脸的新增 L01/L02+ 已生成 `-mesh.png` 变体并上传对象存储（storage，见 AGENTS.md Stage 3 清单 G）
...
> **⚠️ 规则已更新（2026-06-30）**：原"G4 通过即启动下一集 Stage 4"的交叉执行规则已被严格顺序规则替代。当前强制要求：**每集必须完整经历 Stage 4 → G4 → R2 → Stage 5 → G5 全部通过后，方可启动下一集**。此变更的目的是防止 Agent 并行/批量写作多集导致各集脱节、风格不一致。

## 规则

1. **每集独立完成完整循环后方可进入下一集**：EP(N) 必须 G5 ✅ 后，才启动 EP(N+1) 的 Stage 4。不再允许 EP(N) 的 G4 和 EP(N+1) 的 Stage 4 交叉执行。
2. 每集独立经历 Stage 4 → G4 → [G4 合规自检验证] → R2 审核 → Stage 5 → G5 完整循环
3. 集间可能需要更新资产文件：
   - 新场景（新 SCENE-###）→ 更新场景卡片
   - 新角色首次出场 → 确认角色卡片已覆盖
   - 新道具 → 更新道具卡片
...
```
EP01: Stage4 → G4 ✅ → 合规自检 → R2 ✅ → Stage5 → G5 ✅ ──┐
                                                            └→ 启动 EP02（如需新资产先回补资产）
EP02: Stage4 → G4 ✅ → 合规自检 → R2 ✅ → Stage5 → G5 ✅ ──┐
                                                            └→ 启动 EP03（如需新资产先回补资产）
EP03: Stage4 → ...
```

> 每集必须 G5 ✅ 后才能启动下一集的 Stage 4；EP(N+1) 启动前先检查是否需要新资产（新角色/场景/道具）并回补。
...
- ❌ **批量写作多集剧本**（如"写 EP01-EP05"）——严格一次一集，顺序推进
- ❌ **批量生成多集 YAML**（如"生成 EP01-EP05 的 segments.yaml"）——严格一次一集
