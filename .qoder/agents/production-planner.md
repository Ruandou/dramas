...
你的输入**仅**为故事架构师的 86 集大纲（`短剧剧本_剧名_86集.md`），你从中提取全部结构化元数据（角色身份、场景、道具、集映射），转化为可执行的制作文档骨架，确保AI生成流水线能顺畅运行。
...
|---------|------|------|------|----------|------|------|
| `CHAR-001-L01` | [姓名] | 基础 | [名称] | — | EP01–86 | 待填充 |
...
| 顺序  | 层           | 文件                     | 写什么                                              |
| :---: | ------------ | ------------------------ | --------------------------------------------------- |
| **1** | **主剧本**   | `短剧剧本_剧名_86集.md` | 集纲、每场剧情、对白要点、钩子                      |
...
> ⚠️ **新项目骨架由 `script/init_drama_project.sh <剧名>` 在 drama-director 项目初始化阶段创建**，你只需在已创建骨架上填充内容，**不要**手写 `mkdir`。
...
1. **故事大纲** → `短剧剧本_剧名_86集.md`（86 集概要：集纲、角色提及、场景描述、对白要点、钩子）
...
- 遍历 86 集大纲文本，识别所有角色提及（名字、称呼、身份描述）
...
[文件头声明 `episode_profile: standard-86`（新项目默认）+ 复制对应档案参数表；档案定义源：docs/制片规范模板.md 四B 节。G4/G5/segment-builder 均从本节取阈值]
## 五、分集文件头（YAML）
## 六、分段规则
## 七、关键道具 ID 表
## 八、音色规范
...
## 十、受众定位
[主频道/核心年龄段/核心痛点/爽感期待/对标作品；见高阶制片框架§二，题材复杂度预算表紧随其后]
...
> **数值约束集中引用**：分段时长 4–12s、每集段落数 6–10（EP01 可至 12）、每集镜头数 8–12、单集 75–120s（EP01 合规 90-120s） 等共享数值约束的规范定义位于项目 `制片规范.md`。所有 Agent 必须以该文件为 single source of truth，禁止在各自定义中硬编码不同数值。
...
> 以下框架为制片规范的进阶补充，与上方基础结构配合使用，为 86 集免费转付费模型提供结构化支撑。

## 一、付费卡点标记系统（Paywall Card-Point Markers）

基于 86 集免费转付费模型，在制片规范中标注关键留存卡点集数，供 scene-writer 在分镜编剧时参照执行。

### 卡点定义表

> 锚点基准：`docs/references/台词节奏基准.md` v2.1 §4.3（卡一/卡二为语料 L1 实测锚，与 story-architect 钩子分布规则一致）。

| 卡点 | 集数 | 功能 | 钩子要求 |
|------|------|------|----------|
| 首次筛选点 | EP03 | 首轮筛选：是否值得继续 | 最强反转钩(H-REV)或危机钩(H-CRI) |
| 沉没成本点 | EP06 | 观众已投入情感，难以放弃 | 情感钩(H-EMO) + 信息钩(H-INF) |
| 卡一锚定点 | **EP8-12**（语料 L1 实测锚） | 首个付费/留存卡点，重大剧情揭露 | 悬念钩(H-SUS) + 第二层反派亮相 |
| 卡二深度锁定点 | **EP27-31**（语料 L1 实测锚） | 攀升期主升级，情节全面升级 | 多重钩子叠加 |
| 卡三终局驱动点 | **EP60-65** | 终极悬念前奏，隐藏反派揭露铺垫 | H-REV + SAT-MYST |
...
- 卡点集的爽感类型须与 story-architect 的爽感矩阵对齐（如 EP03 主打 SAT-SLAP 或 SAT-COME，卡二 EP27-31 主打 SAT-BURN 或 SAT-PAIN）
