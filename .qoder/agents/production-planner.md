...
| 每集段落数       | 6–10 段         | 对应单集 90 秒（EP01 为 120 秒），参见制片规范 episode_profile |
...
> **数值约束集中引用**：分段时长 4–12s、每集段落数 6–10、每集镜头数 8–12、单集 75–120s（EP01: 90–120s） 等共享数值约束的规范定义位于项目 `制片规范.md`。所有 Agent 必须以该文件为 single source of truth，禁止在各自定义中硬编码不同数值。
