...
4. **不得生成分辨率低于 `engine_registry.video_defaults(aspect_ratio)` 返回的 `image_resolution` 的图片生成参考图**（竖屏 9:16 = 1600×2848，横屏 16:9 = 2848×1600，从 `制片规范.md` → `aspect_ratio` 读取）。视频生成分辨率以 `制片规范.md` 中 `video_resolution` 字段为准（默认 720p）。
