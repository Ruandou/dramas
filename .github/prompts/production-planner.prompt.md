...
# 条件放行协议（Conditional Pass Protocol）
...
> **注意区分两种情况**：
> - **用户未回应**（3 轮用户交互无任何反馈）→ 触发本协议的条件放行流程
> - **用户明确否决**（用户明确表示不通过或指出具体问题）→ 立即返回对应阶段修改，**不进入条件放行流程**
...
- 自检通过的资产标记为「条件放行（⚠️ 待用户确认）」
- 标记必须写入 `工作计划.md`，包含：资产 ID、放行时间、初审结论、放行理由
- 下游阶段可基于条件放行的资产开始工作，但需在 `工作计划.md` 中注明依赖关系
...
- 用户确认通过 → 将「条件放行」更新为「正式通过」
- 用户否决 → 下游已依赖该资产完成的工作需回退重做，资产返回原阶段修改

> **注意**：条件放行是为了避免流水线因等待用户响应而完全停滞，但「条件放行」不等于「正式通过」。若用户后续否决，相关下游工作需回退。
